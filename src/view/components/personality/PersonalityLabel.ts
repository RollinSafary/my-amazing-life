...
export default class PersonalityLabel extends ExtendedText {
    ...
    public updateSector(sectorIndex: number): void {
        this.setText((Translation as any)[`PERSONLAITY_SECTION_TITLE_${this.colorName.toUpperCase()}_${sectorIndex}`])
        const position = personalityLabelsPositions[sectorIndex][this.colorName]
        this.x = this.scene.frameLeft.displayWidth + position.x * gameConfig.designWidth
